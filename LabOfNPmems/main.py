...
def main():
    ...
    # Create save dir 创建保存目录
    create_dirs(args.save_dir)

    # Check CUDA
    if torch.cuda.is_available():#是否有可用GPU
        args.cuda = True
    args.device = torch.device("cuda" if args.cuda else "cpu")#将torch.Tensor分配到的设备对象
    print("Using CUDA: {}".format(args.cuda))#通过格式操作使任意类型的数据转换成一个字符串
    ...
    # train & validation
    print('start MSSTPSA-net pred_seq_len=5s new  training...')
    print('sample_path',args.save_sample_path)
    print('save_dir is: ', args.save_dir)
    ...
    # test_exps = np.load('./result/checkpoint_test/exp_list.npy', allow_pickle=True)
